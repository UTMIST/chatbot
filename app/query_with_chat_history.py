...
def embed_chat_history(chat_history):
    ...
    if isinstance(chat_history, list) and all(
        ...
        raise ValueError(
            "chat_history must be a list of dictionaries with 'content' keys."
        )
...
def ai_response(input, past_chat_history=[]):
...
def classify_relevance(input, retriever=retriever) -> Relevance:
...
def get_response_with_relevance(
    input: str, past_chat_history=[], retriever=retriever
) -> str:
    relevance = classify_relevance(input, retriever=retriever)
    print("relevance: " + str(relevance))
    if relevance == Relevance.KNOWN:
        return ai_response(input, past_chat_history)
...
def get_unknown_response(
    latest_user_input: str, past_chat_history=[], retriever=retriever
) -> str:
...
2. DO NOT make up information that you do not 100% know to be true.
    ...
    return get_openai_response_content(
        system_prompt=UNKNOWN_RESPONSE_PROMPT, messages=messages
    )


def get_openai_response_content(
    system_prompt="", messages=[], model="gpt-3.5-turbo", **kwargs
) -> str:
...
def _extract_openai_response_content(response: ChatCompletion) -> str:
    ...
    ), "response must be a ChatCompletion object"
    return response.choices[0].message.content


def _get_openai_response(
    messages=[], model="gpt-3.5-turbo", **kwargs
) -> ChatCompletion:
    response: ChatCompletion = openai_client_instance.chat.completions.create(
