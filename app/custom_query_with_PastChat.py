...
#Embed and add new documents
documents = SimpleDirectoryReader(input_files=["Your document path"]).load_data()
new_index = VectorStoreIndex.from_documents(documents, embedding_model = embedding_model)
...
class RAGStringQueryEngine(CustomQueryEngine):
    ...
    #custom_query that incorporate chat history
    def custom_query(self, query_str: str, past_chat_history: list):
        ...
        past_nodes = self.retriver.retrive(query_str)
        past_context_str = "\n\n".join([n.node.get_context() for n in past_nodes])
...
def aiResponse(input, past_chat_history  = []):
    ...
    response = query_engine.query(str(input), past_chat_history)
    return response
