    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(
                "/Users/dingshengliu/Desktop/ChatbotAI/Testing/ReadFiles/cred.json",
                SCOPES,
            )
...
def resolve_shortcut(service, file_id):
    try:
        file = (
            service.files()
            .get(fileId=file_id, fields="id, mimeType, shortcutDetails")
            .execute()
        )
        ...
        for item in items:
            ...
            try:
                if mime_type == "application/vnd.google-apps.document":
                    ...
                    export_google_doc(
                        service, file_id, "application/pdf", file_name + ".pdf"
                    )
